...
from bot.shared.constants import EMBED_COLORS, HELP_BANNER_URL
...
class HelpRenderer:
    ...
    async def create_main_embed(
        ...
        """
        if CONFIG.BOT_INFO.BOT_NAME != "Bot":
            embed = self.create_base_embed(
                "Hello! Welcome to the help command.",
                f"{CONFIG.BOT_INFO.BOT_NAME} is a self-hosted instance of Bot. The bot is written in Python using discord.py.\n\nIf you enjoy using {CONFIG.BOT_INFO.BOT_NAME}, consider contributing to the original project.",
            )
        else:
            embed = self.create_base_embed(
                "Hello! Welcome to the help command.",
                "Bot is an all-in-one bot by the AWFixer Enterprising Inc Discord server. The bot is written in Python using discord.py, and we are actively seeking contributors.",
            )
    ...
    async def _add_bot_help_fields(self, embed: discord.Embed) -> None:
        ...
        embed.add_field(
            ...
            value="-# [Help contribute! View Repo](https://github.com/awfixers-stuff/bot)",
            inline=True,
        )
        ...
        bot_name_display = (
            "Bot"
            if CONFIG.BOT_INFO.BOT_NAME == "Bot"
            else f"{CONFIG.BOT_INFO.BOT_NAME} (Bot)"
        )
        owner_info = (
