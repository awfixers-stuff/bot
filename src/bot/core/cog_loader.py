...
from bot.services.sentry.tracing import (
...
from bot.shared.exceptions import BotCogLoadError, BotConfigurationError
...
class CogLoader(commands.Cog):
    ...
    def _is_configuration_error(self, exception: Exception) -> bool:
        ...
        Walks the exception chain to detect BotConfigurationError anywhere in
        ...
        -------
        bool
            True if the exception chain contains a BotConfigurationError.
        """
        current_exception = exception

        while current_exception:
            if isinstance(current_exception, BotConfigurationError):
                return True
            current_exception = (
    ...
    def _resolve_module_path(self, path: Path) -> str:
        ...
        -------
        str
            The Python module path (e.g., "bot.modules.admin.dev").

        Examples
        --------
        >>> loader._resolve_module_path(Path("bot/modules/admin/dev.py"))
        "bot.modules.admin.dev"
        """
    ...
    def _is_duplicate_load(self, module: str) -> bool:
        ...
        "bot.modules.admin" is loaded, this will return True for
        "bot.modules.admin.dev". Checks all parent modules in the path
    ...
    async def _load_single_cog(self, path: Path) -> None:
        ...
        try:
        ...
        except BotConfigurationError as config_error:
            ...
            raise BotCogLoadError(error_msg) from e
    ...
    async def load_cogs(self, path: Path) -> None:
        ...
        try:
            ...
            Exception
        ) as e:  # Catch-all: wrap any cog loading failure into BotCogLoadError
            ...
            raise BotCogLoadError(msg) from e
