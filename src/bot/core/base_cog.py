...
from bot.core.decorators import command_permission_check
from bot.shared.config import CONFIG
from bot.shared.functions import generate_usage
...
class BaseCog(commands.Cog):
        ...
        try:
            for command in self.get_commands():
                ...
                if not getattr(callback, "__uses_dynamic_permissions__", False):
                    continue
                if getattr(callback, "__bot_perm_allow_self_use_for_param__", None):
                    continue
                command.checks.insert(0, command_permission_check)
                callback.__bot_perm_check_in_phase__ = True  # type: ignore[attr-defined]
        except Exception:
            logger.exception(
    ...
    async def is_snippetbanned(self, guild_id: int, user_id: int) -> bool:
        ...
        from bot.database.models import CaseType as DBCaseType  # noqa: PLC0415
