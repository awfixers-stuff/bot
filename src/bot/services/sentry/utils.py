...
def capture_bot_exception(
    ...
    *,
    ...
    with sentry_sdk.push_scope() as scope:
        scope.set_tag("error.type", "bot_error")
        ...
        if command_name:
            bot_context["command"] = command_name
        if user_id:
            bot_context["user_id"] = user_id
        if guild_id:
            bot_context["guild_id"] = guild_id

        scope.set_context("bot_error", bot_context)
        sentry_sdk.capture_exception(error)
...
def convert_httpx_error(
    ...
    *,
    ...
    if isinstance(error, httpx.HTTPStatusError):
        ...
        if status_code == 403:
            raise BotAPIPermissionError(service_name=service_name) from error
