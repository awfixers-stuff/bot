...
from bot.services.handlers.error.formatter import ERROR_CONFIG_MAP
...
class ErrorTestRegistry:
    ...
    def _add_custom_test(self, error_type: type[Exception]) -> None:
        """Add custom error test cases for bot module errors."""
        name = error_type.__name__
        ...
        self.tests[name] = ErrorTestDefinition(
            ...
            realistic_args,
            {},
            f"Custom bot {name.lower().replace('error', ' error')}",
            "Custom",
        )

    def _get_realistic_custom_args(self, error_name: str) -> tuple[Any, ...]:
        """
        Get realistic arguments for custom bot errors.
...
class Mock(BaseCog):
    ...
    async def _send_test_summary(self, ctx: commands.Context[Bot]) -> None:
