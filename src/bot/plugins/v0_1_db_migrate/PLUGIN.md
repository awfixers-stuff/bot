...
The plugin is automatically loaded from `src/bot/plugins/v0_1_db_migrate/`.
...
2. **Stop Old Bot**
   ...
   systemctl stop old-bot  # or however you run it
   ```
...
3. **Set Up New Codebase**
   ...
   git clone <new-repo-url> new-bot
   cd new-bot
   uv sync
...
1. **Set Up Test Environment**
   ...
   ```bash
   # Clone new codebase
   git clone <new-repo-url> test-bot
   cd test-bot
   uv sync
   ```
...
2. **Create Test Database**
   ...
   createdb test_bot_database
   
   # Update .env with test database URL
   # DATABASE_URL=postgresql://user:password@localhost:5432/test_bot_database
