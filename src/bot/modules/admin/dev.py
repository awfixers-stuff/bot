...
class Dev(BaseCog):
    ...
    def _resolve_cog_path(self, cog_name: str) -> str:
        ...
        like "bot.modules.utility.ping" or "bot.plugins.atl.mock" by recursively
        ...
        by prepending "bot." as needed.

        Examples
        --------
        - "ping" → "bot.modules.utility.ping"
        - "mock" → "bot.plugins.atl.mock"
        - "modules.utility.ping" → "bot.modules.utility.ping"
        - "plugins.atl.mock" → "bot.plugins.atl.mock"
        - "bot.modules.utility.ping" → "bot.modules.utility.ping" (unchanged)
        - "something" → "bot.plugins.xyz.something" (if in plugins/xyz/)
        ...
        if "." in cog_name:
            return cog_name if cog_name.startswith("bot.") else f"bot.{cog_name}"

        # Try to find the cog in modules and plugins directories
        bot_dir = Path(__file__).parent.parent.parent  # Go up to bot/

        # Search directories in order of priority: modules first, then plugins
        search_dirs = [
            bot_dir / "modules",  # bot/modules/
            bot_dir / "plugins",  # bot/plugins/
        ]
        ...
        for search_dir in search_dirs:
            ...
            for py_file in search_dir.rglob(f"{cog_name}.py"):
                ...
                try:
                    relative_path = py_file.relative_to(bot_dir)  # From bot/
                    return f"bot.{str(relative_path).replace('/', '.').replace('\\', '.')[:-3]}"
                except ValueError:
                    continue
    ...
    async def stop(self, ctx: commands.Context[Bot]) -> None:
        ...
        """
        await ctx.send(
            "Stopping the bot...\n-# Note: if Bot is running with Docker Compose, this will restart the container.",
        )
