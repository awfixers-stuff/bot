...
class Cases(ModerationCogBase):
    ...
    def _create_case_list_embed(
        ...
        for case in cases:
            ...
            type_emoji_key = CASE_TYPE_EMOJI_MAP.get(case.case_type, "bot_error")
            type_emoji = self.bot.emoji_manager.get(str(type_emoji_key))
            action_emoji_key = CASE_ACTION_MAP.get(case.case_type, "bot_error")
            ...
            if hasattr(case, "created_at") and case.created_at:
                ...
                case_date = f"{self.bot.emoji_manager.get('bot_error')}"
