...
from bot.database.models import Levels
...
class LevelsController(BaseController[Levels]):
    ...
    async def get_levels_by_member(self, member_id: int) -> Levels | None:
        ...
        """
        return await self.find_one(filters=Levels.member_id == member_id)
    ...
    async def get_or_create_levels(self, member_id: int) -> Levels:
        ...
        """
        levels = await self.get_levels_by_member(member_id)
    ...
    async def add_xp(self, member_id: int, xp_amount: float) -> Levels:
        ...
        """
        levels = await self.get_or_create_levels(member_id)
    ...
    async def unblacklist_member(self, member_id: int) -> Levels:
        ...
        levels = await self.get_levels_by_member(member_id)
        if levels is None:
            return await self.get_or_create_levels(member_id)
        return await self.update_by_id(levels.member_id, blacklisted=False) or levels

    async def get_top_members(self, limit: int = 10) -> list[Levels]:
    ...
    async def get_user_level_data(self, member_id: int) -> Levels | None:
        ...
        Levels | None
            The levels record if found, None otherwise.
        """
        return await self.get_levels_by_member(member_id)
    ...
    async def get_all_stats(self) -> dict[str, Any]:
        ...
        """
        total_members = await self.count()
        blacklisted_count = await self.count(
            filters=Levels.blacklisted,
        )
