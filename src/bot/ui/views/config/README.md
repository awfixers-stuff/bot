...
```python
from bot.ui.views.config import ConfigDashboard
...
```python
from bot.ui.views.config.pagination import PaginationHelper
...
```python
from bot.ui.views.config.callbacks import validate_author, handle_callback_error
...
```python
from bot.ui.views.config.helpers import create_back_button, create_error_container
...
Configuration constants are centralized in `bot.shared.constants`:
