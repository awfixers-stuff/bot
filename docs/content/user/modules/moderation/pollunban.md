...
The `pollunban` command (also available as `pub`) allows server moderators to lift a previously applied poll restriction from a member, restoring their ability to use Bot's poll creation features.
...
| `reason` | String | No | The reason for restoring poll access, logged in the moderation case and included in the DM notification. In prefix commands, this is a positional argument. In slash commands, it is a named parameter. Defaults to "No reason provided". |
